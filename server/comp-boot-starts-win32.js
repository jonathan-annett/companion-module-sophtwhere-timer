...
module.exports = function(){
    ...
    const log_path_ok = fs.existsSync(log_path);
 
    const self = {
        log_path : log_path,
        log_path_ok : log_path_ok  ,
        on : function (ev,cb) {
    ...
    function getLastBootTime() {
        ...
        return new Promise(function (resolve,reject){
            ...
            if (log_path_ok) {
                ...
                let last = 0, lastFile;
                self.filenames.forEach(function(file,index){
                    const filename = path.join(self.log_path,file)
                ...
                function returnTheStartTime(lastStartTime) {
                    self.lastAppStart = lastStartTime;
                    self.lastAppTime = lastStartTime.toTimeString();
                    self.lastAppFile = lastFile;
    ...
    function getLogs() {
        ...
        if (self.log_path_ok) {
            if ( self.filenames ) {
                self.filenames.splice(
                    0,
                    self.filenames.length,
                );

                self.filenames.push.apply(
                    self.filenames,
                    readdir()
                );
            } else {
                
                self.filenames = readdir();

            }
            return self.filenames;

            function readdir(){
            return  fs.readdirSync(self.log_path).filter(function(f){
